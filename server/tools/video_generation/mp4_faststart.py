...
def apply_mp4_faststart(file_path: str) -> bool:
    ...
    if ffmpeg:
        ...
        try:
            ...
            print(
                "[faststart] ffmpeg failed, falling back to pure Python: "
            ...
            if os.path.exists(out_path):
                try:
                    ...
                    pass
            print(f"[faststart] ffmpeg error, fallback: {exc}")

    try:
        return _apply_mp4_faststart_python(file_path)
    except Exception as exc:
        print(f"[faststart] skip ({file_path}): {exc}")
        return False
