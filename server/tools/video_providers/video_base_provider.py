...
def get_default_provider(model_info_list: Optional[List[ModelInfo]] = None) -> str:
    ...
    Args:
        model_info_list: List of model info dictionaries. If provided,
                        will prioritize agnes provider if available, otherwise use first one.
                        If not provided, returns 'agnes' as default.
    ...
    if model_info_list:
        for model_info in model_info_list:
            if model_info.get('provider') == 'agnes':
                return 'agnes'

        if model_info_list:
            return model_info_list[0].get('provider', 'agnes')
